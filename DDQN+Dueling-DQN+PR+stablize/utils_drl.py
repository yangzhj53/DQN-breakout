...
class Agent(object):
    ...
    def learn(self, memory: ReplayMemory, batch_size: int) -> float:
        ...
        state_batch, action_batch, reward_batch, next_batch, done_batch, treeNode, ISWeight = memory.sample(batch_size)

        values = self.__policy(state_batch.float()).gather(1, action_batch)
        target_q_value = self.__target(next_batch.float()).detach()
        max_action = self.__policy(next_batch.float()).max(1).unsqueeze(0)
        expected = (self.__gamma * target_q_value.gather(1, max_action)) * (1. - done_batch) + reward_batch
        loss = (ISWeight * F.smooth_l1_loss(values, expected)).mean()
